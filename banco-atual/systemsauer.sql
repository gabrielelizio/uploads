...
CREATE TABLE `pendente` (
	...
	`disciplina` VARCHAR(100) NULL COLLATE 'latin1_swedish_ci',
	...
	`dia_semana` VARCHAR(50) NULL COLLATE 'latin1_swedish_ci',
	`turno` VARCHAR(50) NULL COLLATE 'latin1_swedish_ci'
) ENGINE=MyISAM;
...
INSERT INTO `uploads` (`id`, `id_prof`, `nome`, `proposito`, `email`, `telefone`, `cursos`, `disciplina`, `qtdecopias`, `patharq`, `rastreio`, `data_envio`, `status`, `data_processamento`, `observacao`, `codigos_turmas`, `dia_semana`, `turno`) VALUES
	(110, 328, 'Lucas  Vinicios Martins', 'Parcial 01', 'lucas03188@hotmail.com', '33333333', 'Administracao', 'Contabilidade IntrodutÃ³ria', 45, 'arquivos/1572643222_exerciciosmapadekarnaugh.pdf', '9ed71b89638af21c90560e32c4612ed5', '2019-11-01 19:20:22', 'Impresso', '2019-11-02 02:21:03', '', '152412', 'TerÃ§a-feira', NULL),
	(115, 328, 'Lucas  Vinicios Martins', 'Oficial 02', 'lucas03188@hotmail.com', '33333333', 'SistemasInformacao', 'Engenharia de software', 45, 'arquivos/1572656675_exerciciosmapadekarnaugh.pdf', '6971d6f288905d28ef9fe9a0d6b1f421', '2019-11-01 23:04:35', 'Erros', '2019-11-02 01:55:57', '', '152412', 'TerÃ§a-feira', 'Frente');
...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `tipoCad` int(11) DEFAULT NULL,
  `cpf` varchar(50) DEFAULT NULL,
  `cod_regera` varchar(15) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=330 DEFAULT CHARSET=utf8;

-- Copiando dados para a tabela systemsauer.users: ~82 rows (aproximadamente)
...
INSERT INTO `users` (`id`, `firstname`, `lastname`, `phone`, `email`, `pass`, `tipoCad`, `cpf`, `cod_regera`) VALUES
	...
	(321, 'Admin', 'do Sistema', '31995041815', 'alesauer@gmail.com', '202cb962ac59075b964b07152d234b70', 2, '2147483647', NULL),
	...
	(327, 'Lucas', 'vinicios Martins', '3194047139', 'lucas03188@hotmail.com', '202cb962ac59075b964b07152d234b70', 2, '255255', NULL),
	(328, 'Lucas', ' Vinicios Martins', '33333333', 'lucas03188@hotmail.com', 'caf1a3dfb505ffed0d024130f58c5cfa', 1, '128', NULL),
	(329, 'Egidio', 'martins', '3125370329', 'egideus@gmail.com', '43f6a40db954b4913c47ed60fa665bf9', 2, '123456789', NULL);
/*!40000 ALTER TABLE `users` ENABLE KEYS */;

-- Copiando estrutura para view systemsauer.pendente
-- Removendo tabela temporária e criando a estrutura VIEW final
DROP TABLE IF EXISTS `pendente`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pendente` AS select `uploads`.`id` AS `id`,`uploads`.`id_prof` AS `id_prof`,`uploads`.`nome` AS `nome`,`uploads`.`proposito` AS `proposito`,`uploads`.`email` AS `email`,`uploads`.`telefone` AS `telefone`,`uploads`.`cursos` AS `cursos`,`uploads`.`disciplina` AS `disciplina`,`uploads`.`qtdecopias` AS `qtdecopias`,`uploads`.`patharq` AS `patharq`,`uploads`.`rastreio` AS `rastreio`,`uploads`.`data_envio` AS `data_envio`,`uploads`.`status` AS `status`,`uploads`.`data_processamento` AS `data_processamento`,`uploads`.`observacao` AS `observacao`,`uploads`.`codigos_turmas` AS `codigos_turmas`,`uploads`.`dia_semana` AS `dia_semana`,`uploads`.`turno` AS `turno` from `uploads` where (`uploads`.`status` = 'Pendente') order by `uploads`.`nome` ;

-- Copiando estrutura para view systemsauer.processada
-- Removendo tabela temporária e criando a estrutura VIEW final
DROP TABLE IF EXISTS `processada`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `processada` AS select `uploads`.`id` AS `id`,`uploads`.`id_prof` AS `id_prof`,`uploads`.`nome` AS `nome`,`uploads`.`proposito` AS `proposito`,`uploads`.`email` AS `email`,`uploads`.`telefone` AS `telefone`,`uploads`.`cursos` AS `cursos`,`uploads`.`disciplina` AS `disciplina`,`uploads`.`qtdecopias` AS `qtdecopias`,`uploads`.`patharq` AS `patharq`,`uploads`.`rastreio` AS `rastreio`,`uploads`.`data_envio` AS `data_envio`,`uploads`.`status` AS `status`,`uploads`.`data_processamento` AS `data_processamento`,`uploads`.`observacao` AS `observacao`,`uploads`.`codigos_turmas` AS `codigos_turmas`,`uploads`.`dia_semana` AS `dia_semana`,`uploads`.`turno` AS `turno` from `uploads` where (`uploads`.`status` = 'Impresso') order by `uploads`.`nome` ;
